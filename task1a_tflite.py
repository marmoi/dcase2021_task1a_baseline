...
from model_size_calculation import get_lite_model_size, get_keras_model_size
    ...
    for parameter_set in parameters_sets:
        ...
        if application_mode == 'dev':
            ...
            if param.get_path('flow.testing'):
                ...
                processed_items = do_quantized_testing(
                    db=db,
...
def do_learning(db, folds, param, log, overwrite=False):
    ...
    for fold in folds:
        ...
        fold_model_quant_filename = os.path.join(
            param.get_path('path.application.learner'),
            'model_quant_fold_{fold}.tflite'.format(fold=fold)
        )
        if not os.path.isfile(fold_model_quant_filename) or overwrite:
            log.line()
            ...
            if param.get_path('learner.parameters.generator.enable'):
                ...
                X_train, Y_train, data_size = dcase_util.tfkeras.data_collector(
                    item_list=item_list_train,
                    data_processing_chain=data_processing_chain,
                    meta_processing_chain=meta_processing_chain,
                ...
                if item_list_validation:
                    ...
                    X_validation, Y_validation, data_size = dcase_util.tfkeras.data_collector(
                        item_list=item_list_validation,
                        data_processing_chain=data_processing_chain,
                        meta_processing_chain=meta_processing_chain,
                        target_format=param.get_path('learner.parameters.data.target_format',
            ...
            with open(fold_model_quant_filename, "wb") as output_file:
                output_file.write(tflite_model)

            processed_files.append(fold_model_quant_filename)

    return processed_files


def do_quantized_testing(db, scene_labels, folds, param, log, overwrite=False):
    """Testing stage
    ...
    for fold in folds:
        ...
        interpreter = tf.lite.Interpreter(model_path=fold_model_quant_filename)
        ...
        if not os.path.isfile(fold_results_filename) or overwrite:
            ...
            for item in db.test(fold=fold):
                ...
                if len(interpreter.get_input_details()[0]['shape']) == 4:#if input_index == 4:
                    data_format = param.get_path('learner.parameters.data.data_format')  # None

                    # Add channel
                    if data_format == 'channels_first':
                        input_data = numpy.expand_dims(input_data, 0).astype(numpy.float32)

                    elif data_format == 'channels_last':
                        input_data = numpy.expand_dims(input_data, 3).astype(numpy.float32)
...
def do_model_size_calculation(db, folds, param, log):
    ...
    for fold in folds:
        ...
        model_size = get_lite_model_size(interpreter, verbose=True, ui=log)
